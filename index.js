module.exports = function(){//dependancies
    ...
    Phrowser.prototype.init=function(readyCB){
    	...
    	self.PH_server=phantom.create(function(ph){
    		self.PH_js_obj=ph;
    ...
    if(self.init_url!==false){
        var r_url_obj=url.parse(req.url),
            init_url_obj=url.parse(self.init_url);
    ...
    self.do_debug(self.debug_levels.all) && console.log("\n\n================== PHROWSER  onResourceRequested ============="+"\n\t"," req.ur: ",req.url,"=============================================================\n\n");
                ...
                self.headers.push(merge(true,{},self.headers_schema,this_header));
            ...
            self.PH_webpage_obj.set('onResourceReceived', function (res) {
                if (res.stage == 'end') {
    if(self.init_url!==false){
        var r_url_obj=url.parse(res.url),
            init_url_obj=url.parse(self.init_url);
        if(typeof(r_url_obj.url)==='string' && r_url_obj.host==init_url_obj.host){
    self.do_debug(self.debug_levels.all) && console.log("\n\n================== PHROWSER  onResourceReceived ============="+"\n\t"," res.url: ",res.url,"=============================================================\n\n");
                    ...
                    self.headers.push(merge(true,{},self.headers_schema,this_header));
                    if(utils.obj_valid_key(res, 'redirectURL')){
...
self.do_debug(self.debug_levels.all) && console.log("===== PHROWSER onLoadFinished: " +
    ...
    Phrowser.prototype.add_event_link=function(typeIn,dataIn){
    	var self=this,
    		new_link=merge(true,{'type':typeIn},(typeof(dataIn)=='object' && dataIn!==null?dataIn:{}));
    ...
    Phrowser.prototype.open=function(reqUrl, opts){
    	var self=this,
            ...
            open_opts=merge(true, open_defaults, (typeof(opts)==='object'?opts:{})),
    		open_url=reqUrl,
            open_method='GET';

        if(open_opts.method.toUpperCase()==='POST'){open_method='POST';}
        open_opts.data.GET=(typeof(open_opts.data.GET)==='string'?querystring.parse(open_opts.data.GET):open_opts.data.GET);//should be object
        open_opts.data.POST=(typeof(open_opts.data.POST)==='string'?querystring.parse(open_opts.data.POST):open_opts.data.POST);//should be object
        ...
        if(open_url.indexOf('?')!==-1){//does get string exist in the URL - Merge it in!
            ...
            var qry_obj=querystring.parse(_ex.slice(1, _ex.length).join('?'));
            open_opts.data.GET=merge(true, {}, qry_obj, (typeof(open_opts.data.GET)==='object'?open_opts.data.GET:{}));
        }
        if(open_method==='POST' && typeof(open_opts.data.POST)==='object'){//if POST! and OBJECT
            open_opts.data.POST=querystring.stringify(open_opts.data.POST);}

        var get_str=(typeof(open_opts.data.GET)==='object'?'?'+querystring.parse(open_opts.data.GET):'?'+utils.check_strip_first(open_opts.data.GET, '?'));
        open_opts.method=open_method;
        open_url=(open_opts.method.toUpperCase()!=='GET' && utils.basic_str(get_str)?open_url+get_str:open_url);//if not GET, set the URL to include the GET data
        ...
        if(utils.basic_str(open_opts.referer)){
            self.PH_webpage_obj.customHeaders = merge(true,{},self.PH_webpage_obj.customHeaders,{'Referer': open_opts.referer});}

        if(open_opts.data.COOKIES instanceof Array && open_opts.data.COOKIES.length>0){
            var url_obj=url.parse(open_url);
            url_obj.host='.'+utils.check_strip_first(utils.check_strip_first(url_obj.host, 'www.'), '.');
            ...
            for(var i=0;i<open_opts.data.COOKIES.length;i++){
                if(utils.obj_valid_key(open_opts.data.COOKIES[i],'name') && utils.obj_valid_key(open_opts.data.COOKIES[i],'value') && utils.obj_valid_key(open_opts.data.COOKIES[i],'path')){
                    var this_cookie=merge(true, {}, cookie_schema, open_opts.data.COOKIES[i]);
...
console.log('vis',vis.length);
    		...
    		var the_olds=_.difference(self.mutables.visibles,visibles),
    			the_news=_.difference(visibles,self.mutables.visibles),
...
console.log('========VISIBLES============',(_.isEqual(self.mutables.visibles, visibles)?'NO CHANGE':'----CHANGED---'));
    if(list.length===0){return;}
    //console.log('list',list,"\n\n\n",'the_olds',the_olds,"\n\n\n",'the_news',the_news);
    	for(var o=0;o<list.length;o++){
    		if(_.contains(the_olds,list[o])){console.log('======found======',list[o].nodeType,' id=',(list[o].id?list[o].id:'NOID'),' class=',(list[o].className?list[o].className:'NOID'));}
...
console.log('========HIDDENS============',(_.isEqual(self.mutables.hiddens, hiddens)?'NO CHANGE':'----CHANGED---'));
